...
def seed(db: sqlite3.Connection):
    ...
    media = [
        ["m1", "ev-cultural", "Opening performance", "photo", "public", "https://images.unsplash.com/photo-1501281668745-f7f57925c3b4?auto=format&fit=crop&w=900&q=80", "Meera Nair", "2026-02-18", ["stage", "crowd", "culture", "night"], ["navya", "meera"], 124, ["Perfect cover shot.", "The lighting is excellent."], 18, "A high-energy stage opener with a packed student crowd.", "approved", "Safe", 8, "s3://eventvault-originals/cultural/opening.jpg"],
        ["m2", "ev-cultural", "Backstage portrait", "photo", "private", "https://images.unsplash.com/photo-1529156069898-49953e39b3ac?auto=format&fit=crop&w=900&q=80", "Aarav Sharma", "2026-02-18", ["portrait", "friends", "backstage"], ["navya", "aarav"], 89, ["Keep this in member gallery."], 22, "A private backstage friendship portrait before the performance.", "review", "Contains private faces", 12, "s3://eventvault-private/cultural/backstage.jpg"],
        ["m3", "ev-mountain", "Ridge trail group", "photo", "public", "https://images.unsplash.com/photo-1519681393784-d120267933ba?auto=format&fit=crop&w=900&q=80", "Rohan Iyer", "2026-01-12", ["mountains", "travel", "group", "sunset"], ["rohan", "isha"], 146, ["Great for the trip recap."], 34, "Students pause on a ridge trail during golden-hour photography.", "approved", "Safe", 4, "s3://eventvault-originals/trip/ridge.jpg"],
        ["m4", "ev-workshop", "Prompt lab session", "photo", "private", "https://images.unsplash.com/photo-1531482615713-2afd69097998?auto=format&fit=crop&w=900&q=80", "Isha Kapoor", "2026-03-04", ["workshop", "ai", "students", "laptop"], ["navya", "isha"], 71, ["Add to workshop highlights."], 15, "Members test AI tagging prompts during a hands-on media lab.", "approved", "Safe", 6, "s3://eventvault-originals/workshop/prompt-lab.jpg"],
        ["m5", "ev-sports", "Final goal celebration", "photo", "public", "https://images.unsplash.com/photo-1521412644187-c49fa049e84d?auto=format&fit=crop&w=900&q=80", "Kabir Menon", "2026-03-21", ["sports", "team", "action", "crowd"], ["kabir", "aarav"], 201, ["Best sports album thumbnail."], 46, "A match-winning goal celebration captured from the sideline.", "duplicate", "Possible duplicate burst", 91, "s3://eventvault-originals/sports/final-goal.jpg"],
        ["m6", "ev-workshop", "Cloud upload walkthrough", "video", "public", "https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4", "Meera Nair", "2026-03-04", ["cloud", "demo", "workshop", "video"], ["meera"], 53, ["Trim intro before publishing."], 9, "A short demo clip explaining direct-to-S3 uploads.", "review", "Needs trim approval", 0, "s3://eventvault-originals/workshop/cloud-demo.mp4"],
        ["m7", "ev-mountain", "Campfire circle", "photo", "private", "https://images.unsplash.com/photo-1500534314209-a25ddb2bd429?auto=format&fit=crop&w=900&q=80", "Rohan Iyer", "2026-01-12", ["travel", "camp", "friends", "night"], ["navya", "rohan", "isha"], 64, ["Private album only."], 19, "A member-only campfire photo from the photography trip.", "review", "Private night scene", 18, "s3://eventvault-private/trip/campfire.jpg"],
        ["m8", "ev-sports", "Trophy lift", "photo", "public", "https://images.unsplash.com/photo-1547347298-4074fc3086f0?auto=format&fit=crop&w=900&q=80", "Kabir Menon", "2026-03-21", ["sports", "trophy", "team", "celebration"], ["kabir", "navya"], 173, ["Use in annual report."], 38, "The winning team lifts the trophy in front of club members.", "approved", "Safe", 7, "s3://eventvault-originals/sports/trophy.jpg"],
    ]
    for item in media:
        db.execute(
    ...
    add_notification(db, "A QR album link was created for Inter-Club Sports League.")
...
def bootstrap():
    with connect() as db:
        ...
        for row in db.execute("SELECT * FROM events ORDER BY date DESC"):
            ...
            events.append(event)
        media = [media_row(row) for row in db.execute("SELECT * FROM media ORDER BY date DESC, id DESC")]
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            if parsed.path == "/api/events":
                ...
                event_id = read_json(self).get("eventId")
                with connect() as db:
                    event = db.execute("SELECT name FROM events WHERE id=?", (event_id,)).fetchone()
                    if not event:
                        json_response(self, {"error": "Event not found"}, 404)
                        return
                    add_notification(db, f"QR album link copied for {event['name']}.")
                json_response(self, {"link": f"/index.html#gallery?event={event_id}", **bootstrap()})
            else:
    ...
    def create_event(self, payload):
        ...
        with connect() as db:
            db.execute(
                ...
                (
                    ...
                    payload["date"],
                    "New collaborative event album ready for uploads, AI indexing, QR sharing, and access rules.",
        ...
        json_response(self, bootstrap(), 201)
...
if __name__ == "__main__":
    ...
    print(f"EventVault running at http://127.0.0.1:{port}/index.html")
